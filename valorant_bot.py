# -*- coding: utf-8 -*-
"""
Valorant専用 PekoriBot v1.1 モジュール
 - /peko rank : ランク登録
 - /peko team : VCメンバーでチーム分け
 - /peko teamtest : 固定10人でチーム分けテスト
"""
...
import itertools
import logging
from google_slide import generate_team_image


# ============================================================
# ⚙️ Bot セットアップ関数（main.pyから呼び出し）
# ============================================================
def setup_bot():
    ...
    register_valorant_commands(bot)
    return bot


# ============================================================
# 🧩 PekoriBot (Valorant) のコマンド登録
# ============================================================
def register_valorant_commands(bot):
    ...
    GAS_WEBHOOK_URL = (
        ...
        "AKfycbztYZmisYPC_BbyY-lNG296sIQHZBo_iu1xMcf8M_5_QJX7DGUNcz5Z2HP2gWgW-RvvEg/exec"
    )
    ...
    async def rank(
        ...
        RANK_NORMALIZE = {
            ...
            r"^(silver|しるばー|シルバー|シル|汁)": "シルバー",
            r"^(gold|ごーるど|ゴールド|ゴル|ごる)": "ゴールド",
            r"^(plat|platinum|ぷらちな|プラ|プラチナ)": "プラチナ",
            r"^(dia|diamond|だいや|ダイヤ|ダイヤモンド)": "ダイヤモンド",
            r"^(ase|ascendant|アセ|アセンダント)": "アセンダント",
            r"^(imm|immortal|いも|イモータル)": "イモータル",
            r"^(rad|radiant|レディ|レディアント)": "レディアント",
        }
    ...
    async def team(ctx):
        ...
        embed.add_field(name="🟥 アタッカー", value="\n".join(
            [f"{get_rank_emoji(p[1], emoji_dict)} {p[0]}" for p in teamA]
        ) + f"\nポイント：{powerA}", inline=True)
        embed.add_field(name="🟦 ディフェンダー", value="\n".join(
            [f"{get_rank_emoji(p[1], emoji_dict)} {p[0]}" for p in teamB]
    ...
    async def on_ready():
        ...
        logging.info(f"✅ Ready as {bot.user}")
